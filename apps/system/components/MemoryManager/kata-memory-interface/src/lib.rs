...
impl ObjDescBundle {
    ...
    pub fn move_objects_to_toplevel(&mut self) -> seL4_Result {
        ...
        for od in &mut self.objs {
            let dest_slot = unsafe { KATA_CSPACE_SLOTS.alloc(od.retype_count()) }
...
#[inline]
pub fn kata_object_alloc(request: &ObjDescBundle) -> Result<(), MemoryManagerError> {
    extern "C" {
    ...
    trace!("kata_object_alloc {}", request);
...
#[inline]
pub fn kata_object_alloc_in_toplevel(
    objs: Vec<ObjDesc>,
    ...
    match request.move_objects_to_toplevel() {
        Err(_) => {
            kata_object_free(&request).expect("kata_object_alloc_in_toplevel");
...
#[inline]
pub fn kata_object_alloc_in_cnode(objs: Vec<ObjDesc>) -> Result<ObjDescBundle, MemoryManagerError> {
    ...
    match kata_object_alloc(&request) {
        Err(e) => {
            kata_object_free_toplevel(&cnode).expect("cnode free");
            Err(e)
        }
        Ok(_) => Ok(request),
    }
...
#[inline]
pub fn kata_untyped_alloc(space_bytes: usize) -> Result<ObjDescBundle, MemoryManagerError> {
...
// Like kata_frame_alloc but also create a CNode to hold the frames.
#[inline]
...
#[inline]
pub fn kata_object_free(request: &ObjDescBundle) -> Result<(), MemoryManagerError> {
    extern "C" {
        // NB: this assumes the MemoryManager component is named "memory".
        fn memory_free(c_data_len: u32, c_data: *const u8) -> MemoryManagerError;
    }
    trace!("kata_object_free {}", request);
    let raw_data = &mut [0u8; RAW_OBJ_DESC_DATA_SIZE];
    postcard::to_slice(request, &mut raw_data[..])
        .map_err(|_| MemoryManagerError::MmeSerializeFailed)?;
    unsafe {
...
// kata_object_alloc_in_cnode).
#[inline]
pub fn kata_object_free_in_cnode(request: &ObjDescBundle) -> Result<(), MemoryManagerError> {
