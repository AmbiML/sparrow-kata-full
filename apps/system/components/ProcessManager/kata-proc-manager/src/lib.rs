...
// a two-step dance to setup an instance because we embed KataManagerInterface
// in the same struct and because we want KATA_PROC static and ProcessManager
...
mod tests {
    ...
    #[test]
    fn test_pkg_mgmt() {
        let mut mgr = KataProcManager::empty();
        mgr.init();
